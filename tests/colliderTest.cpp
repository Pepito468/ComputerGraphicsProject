...
#include "../src/engine/Collider.hpp"
#include <iostream>
...
void boxTest()
{
    ...
    for (float x = -1; x < 1; x += 0.1f)
    {
        for (float y = -1; y < 1; y += 0.1f)
        {
            for (float z = -1; z < 1; z += 0.1f)
            {
                if (y >= abs(x) - d && y <= -abs(x) + d && abs(z) <= 0.5f)
                {
                    assert(box->inBounds(glm::vec3(x, y, z)));
                }
                else
    ...
    for (float x = -1.5f; x < 1.5f; x += 0.1f)
    {
        for (float y = -1; y < 1; y += 0.1f)
        {
            for (float z = -1; z < 1; z += 0.1f)
            {
                if (abs(x) <= 1 && abs(y) <= 0.5f && abs(z) <= 0.5f)
                {
                    assert(box->inBounds(glm::vec3(x, y, z)));
                }
                else
...
int main()
{
    std::cout << std::endl << "COLLIDER TEST" << std::endl;
    boxTest();
    std::cout << std::endl << "END COLLIDER TEST" << std::endl;
}
