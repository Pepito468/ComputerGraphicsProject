...
vec3 iblDiffuse(vec3 N, vec3 albedo, vec3 F, float metallic) {
    ...
    vec3 iblDiffuseC = (vec3(1.0)-F)*(1.0-metallic)*albedo*irradiance;
    return iblDiffuseC;
}
...
vec3 iblSpecular(vec3 N, vec3 V, float roughness, vec3 F, float NdotV) {
    vec3 r = -reflect(V,N);
    vec3 prefilteredColor = textureLod(envMap,r, roughness*MAX_REFLECT_LOD).rgb;
    vec2 AB = envBRDFApprox(roughness, NdotV);
    return prefilteredColor * (F*AB.x + AB.y);
}*/
...
void main() {
    ...
    vec3 ambient = (iblDiffuse(N, albedo, F_ibl, metallic)+
    iblSpecular(N, V, roughness, F_ibl, NdotV))*ao;
    */
